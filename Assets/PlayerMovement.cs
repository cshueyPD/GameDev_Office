...
public class PlayerMovement : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(gameManager.Instance.canFire)
        {
            ...
            {
                gameManager.Instance.currentProjectile = ProjectileType.RED;
            }
            else if (Input.GetKeyDown("2"))
            {
                gameManager.Instance.currentProjectile = ProjectileType.GREEN;
            }
            else if (Input.GetKeyDown("3"))
            {
                gameManager.Instance.currentProjectile = ProjectileType.DONUT;
            }

            fireRate = projectile[(int)gameManager.Instance.currentProjectile].GetComponent<Projectile_Motion>().fireRate;
    ...
    private void Shoot()
    {
        ...
        shootingSource.clip = shootingSounds[(int)gameManager.Instance.currentProjectile];
        shootingSource.Play();
        Instantiate(projectile[(int)gameManager.Instance.currentProjectile], firingPoint.position, firingPoint.rotation);
    }
